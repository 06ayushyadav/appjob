...
const Card = ({ id, title, skills, salary, org, onDelete }) => {
    const handleDelete = async () => {
        try {
            const response = await fetch(`https://appjob-o3ho.onrender.com/api/delete/${id}`, {
                method: "DELETE",
                credentials: "include",
            ...
            <div className='w-80 h-auto rounded-lg shadow-lg border border-blue-600 mt-5 '>
                ...
                <div className='flex justify-center items-center gap-10 mt-5'>
                    ...
                    <button className="mb-3" onClick={handleDelete}>
                        <MdDelete className="w-10 h-8  text-red-600 " title="Delete Job" />
                    </button>
                </div>
            </div>
        </>
...
function JobCard() {
    ...
    const fetchJobs = async () => {
        try {
            const response = await fetch("https://appjob-o3ho.onrender.com/api/upload-job");
            const data = await response.json();
            if (data.success) {
                setJobs(data.jobs);
            } else {
